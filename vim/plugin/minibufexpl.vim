...
" Added the following auto commands to periodically update the mini buffer
" explorer on cursor idleness
autocmd MiniBufExplorer CursorHold  * call <SID>DEBUG('-=> CursorHold  AutoCmd', 10) |call <SID>AutoUpdate(-1)
autocmd MiniBufExplorer CursorHoldI * call <SID>DEBUG('-=> CursorHoldI  AutoCmd', 10) |call <SID>AutoUpdate(-1)
" }}}
...
function! <SID>BufMod()
    if &modified == 1
        call <SID>DEBUG('-=> BufMod AutoCmd', 10) |call <SID>AutoUpdate(-1)
    endif
endfunction 
